...
def getCountryListOnline():
    ...
    pays_options = soup.find(
        'div', {'class': 'select_location_form standard_margin'}).find_all('option')[1:]
    pays = [x.text for x in pays_options]
    # on remplace aussi les espaces par des tirets dans les noms pour pouvoir construire les urls par la suite
    ...
    if lignes:
        ...
        for ligne in lignes:
            if ligne.find_all('td'):
                ...
                df = pd.DataFrame(data, columns=['classement', 'ville', 'idx_cout_vie', 'idx_loyer',
                                  'idx_cout_vie_loyer', 'idx_courses', 'idx_prix_restaurants', 'idx_pouvoir_achat_local'])
                df['Pays'] = soup.find(
                    'input', {'name': 'locCountry'})['value']
    else:
        return -1
    return df
...
def modifCoordinatesV2():
    ...
    # Quebec City
    df.loc[df["ville"] ==
           "La ville de Québec (existing value)", "lat"] = 46.81213588141209
    df.loc[df["ville"] ==
           "La ville de Québec (existing value)", "lon"] = -71.22480805636772
    # Puebla Mexique:
